...
export class OrganizersComponent implements OnInit {
  ...
  getEvent(event: any = ''): void {
    ...
    this._organizersService.eventsList(filter).subscribe((result: any) => {
  ...
  liveEvent(event: any, eventObj: any, index: number): void {
    ...
    if (eventObj.is_approved) {
      this.isLoading = true;
      this._organizersService.liveEventById(eventObj._id).subscribe((result: any) => {
        if (result && result.IsSuccess) {
  ...
  multipleLiveEvent(): void {
    if (this.selectedEventIds && this.selectedEventIds.length) {
      this.isLoading = true;
      this._organizersService.liveMultipleEvents(this.selectedEventIds).subscribe((result: any) => {
        if (result && result.IsSuccess) {
